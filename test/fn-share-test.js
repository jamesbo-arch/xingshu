...
// 测试用户以 test_share_ 前缀创建，故事为 private 不污染广场，结束时硬删。
...
async function run() {
  ...
  async function shareCount(storyId) {
    const [[row]] = await conn.query('SELECT share_count FROM stories WHERE id = ?', [storyId])
    return row.share_count
  }

  // 准备：登录建用户 → 建私密故事（初始 share_count=0）
  ...
  const story = await callFn('createStory', {
    title: '分享计数测试故事', content: '本条由 fn-share-test 创建，若残留请删除',
    tags: [], publishStatus: 'draft',
  }, OPENID)
  const storyId = story.data.id

  await test('SHARE-A01 初始分享数为 0', async () => {
    if (await shareCount(storyId) !== 0) throw new Error('新建故事 share_count 应为 0')
  })

  await test('SHARE-A02 记录一次分享 → share_count+1 且返回最新值', async () => {
    const r = await callFn('recordShare', { storyId }, OPENID)
    if (r.code !== 0) throw new Error(r.msg)
    if (r.data.shares !== 1) throw new Error(`返回 shares=${r.data.shares}，期望 1`)
    if (await shareCount(storyId) !== 1) throw new Error('share_count 未 +1')
  })

  await test('SHARE-A03 同一用户重复分享幂等，不重复计数', async () => {
    await callFn('recordShare', { storyId }, OPENID)
    const r = await callFn('recordShare', { storyId }, OPENID)
  ...
  await test('SHARE-A04 另一用户分享 → 去重分享人 +1', async () => {
    ...
    const r = await callFn('recordShare', { storyId }, 'test_share_u2')
    if (r.data.shares !== 2) throw new Error(`期望 2，实际 ${r.data.shares}`)
    const [[row]] = await conn.query(
      "SELECT COUNT(*) n FROM interactions WHERE target_type='story' AND target_id=? AND action='share'", [storyId])
    if (row.n !== 2) throw new Error(`互动行 ${row.n} 条，期望 2`)
  })

  await test('SHARE-A05 缺 storyId 拒绝', async () => {
    const r = await callFn('recordShare', {}, OPENID)
    if (r.code === 0) throw new Error('缺 storyId 不应通过')
  })

  await test('SHARE-A06 故事不存在拒绝', async () => {
    const r = await callFn('recordShare', { storyId: 999999999 }, OPENID)
    if (r.code === 0) throw new Error('不存在的故事不应通过')
  })

  await test('SHARE-A07 未注册用户拒绝', async () => {
    const r = await callFn('recordShare', { storyId }, 'test_share_ghost_not_exist')
    if (r.code === 0) throw new Error('未注册用户不应通过')
  })

  // 清理（interactions 先删，再删故事与用户）
  await conn.query("DELETE FROM interactions WHERE target_type='story' AND target_id = ?", [storyId])
