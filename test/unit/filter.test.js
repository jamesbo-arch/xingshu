...
const STORIES = [
  { id: 1, title: '晨跑记录', content: '今天跑了五公里', author: '砚秋', tags: ['运动'], publishStatus: 'published', isFavorited: true, isMine: false, timestamp: daysAgo(0) },
  { id: 2, title: '读书笔记', content: '读完了一本小说', author: '我', tags: ['读书'], publishStatus: 'draft', isFavorited: false, isMine: true, timestamp: daysAgo(3) },
  { id: 3, title: '旅行故事', content: '去了海边', author: '陆明远', tags: ['旅行', '摄影'], publishStatus: 'published', isFavorited: false, isMine: false, timestamp: '2025-01-15 09:00' },
  { id: 4, title: '他人暂存', content: '不应出现在广场', author: '叶清和', tags: [], publishStatus: 'draft', isFavorited: true, isMine: false, timestamp: daysAgo(1) },
]
...
test('collections 模式只保留已收藏', () => {
  const r = applyFilters(STORIES, 'collections', '', NO_FILTERS)
  assert.deepStrictEqual(r.map(d => d.id), [1, 4])
})

test('mine 模式只保留自己的故事', () => {
  const r = applyFilters(STORIES, 'mine', '', NO_FILTERS)
