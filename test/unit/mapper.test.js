...
test('story：时间按字符串字面解析（DB 已存北京时间，不做时区换算）', () => {
  const d = mapper.story({ created_at: '2026-06-19T12:04:28.000Z' })
  assert.strictEqual(d.timestamp, '2026-06-19 12:04')  // 字面 12:04，不偏移
  assert.strictEqual(d.dateText, '2026-06-19')
})

test('story：like_count 为 0 时不回退到 likes 字段', () => {
  const d = mapper.story({ like_count: 0, likes: 99 })
  assert.strictEqual(d.likes, 0)
})

test('story：非会员作者 authorIsMember 为 false', () => {
  assert.strictEqual(mapper.story({ author_identity: 'authed' }).authorIsMember, false)
  assert.strictEqual(mapper.story({}).authorIsMember, false)
})

test('story：images 缺失时兜底为空数组，存在时透传', () => {
  assert.deepStrictEqual(mapper.story({}).images, [])
  assert.deepStrictEqual(mapper.story({ images: ['cloud://a.jpg'] }).images, ['cloud://a.jpg'])
})

test('story：null/undefined 原样返回', () => {
  assert.strictEqual(mapper.story(null), null)
  assert.strictEqual(mapper.story(undefined), undefined)
})
...
test('story：created_at_text 对久远日期输出 MM-DD HH:MM', () => {
  const d = mapper.story({ created_at: '2020-03-05 14:30:00' })
  assert.strictEqual(d.created_at_text, '03-05 14:30')
})

test('story：created_at 为空时 created_at_text 为空字符串', () => {
  assert.strictEqual(mapper.story({}).created_at_text, '')
})
