...
function App({ tweak }) {
  ...
  const [user, setUser] = React.useState(() => {
    // Read admin-created order sync from localStorage
    try {
      const raw = localStorage.getItem('xingshu_admin_sync');
      if (raw) {
        ...
        if (sync?.user) {
          return {
            ...window.CURRENT_USER,
            ...sync.user,
            latestOrder: sync.order ? {
              ...sync.order,
              method: sync.order.method ? `线下 · ${sync.order.method}` : '线下支付',
            } : window.CURRENT_USER.latestOrder,
          };
        }
      }
    } catch (e) {}
  ...
  // listen for admin order creation (cross-tab via storage event)
  React.useEffect(() => {
    const handler = (e) => {
      if (e.key !== 'xingshu_admin_sync' || !e.newValue) return;
      try {
        ...
        if (sync?.user) {
          ...
          showToast(`已收到管理员开通的会员订单 ${sync.order?.id || ''}`);
        }
      } catch (err) {}
    };
...
function Root() {
  ...
  // Detect if we're on a small/phone viewport — drop the device chrome there
  ...
  // On phone: render the app fullscreen (no chrome, no bg pattern)
  if (isPhone) {
    return (
      <div style={{
        width: '100vw', height: '100vh', background: 'var(--paper)',
  ...
  // On desktop: render inside an iPhone frame, with Tweaks panel
  return (
