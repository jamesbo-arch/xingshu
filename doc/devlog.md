...
### 2026-07-18 — 故事详情页禁止框选复制正文

**类型**：前端
**计划关联**：用户需求（详情页不允许框选复制文字）
**修改文件**：
...
### 2026-07-19 — 现场分享宫格统一为 3 列等大缩略图

**类型**：前端
**计划关联**：用户反馈（除单张与视频外，缩略图一律等大九宫格；如 6 张为 2×3）
**修改文件**：
- `miniprogram/pages/activity-detail/index.js` — gridClass 去掉 4 张田字特例，改为「1 张 pg1 大图，其余一律 pg3」
- `miniprogram/pages/activity-detail/index.wxss` — 上版 216rpx 固定格 3 列共 664rpx 超过内容区（638rpx）致折成 2 列；改 206rpx 格（3×206+2×8=634≤638 恒 3 列不折），删除 pg3/pg4 定宽与田字
...
### 2026-07-20 — 取消「我的收藏」页签，入口移入会员中心

**类型**：前端 | 文档
**计划关联**：用户需求（收藏是低频个人入口，不该占一级导航位）
**修改文件**：
- `miniprogram/pages/member/index.wxml` — authed 与 member 两个分支各插入一处「我的收藏 ›」入口行（个人资料卡下方、互动统计上方），复用 `.section-card` + `.settings-row` + 全局 `.ic-chevron`，不新造样式
- `miniprogram/pages/member/index.js` — 加 `onOpenCollections()` → `navigateTo('/pages/collections/index')`
- `miniprogram/pages/collections/index.wxml/.wxss` — 页头加返回箭头（`.nav-back` + `ic-back`，**显式给 40rpx 宽高**，否则背景图类 0×0 不可见）；`.list-bottom-pad` 去掉 100rpx 的 tab-bar 让位
- `miniprogram/pages/collections/index.js` — 删 onShow 的 `getTabBar().refresh()`；删 `_tabBar()` 及 6 处调用（非 tab 页后为死代码）；加 `onBack()`；`onGuardJoinMember` 由 switchTab 改 `navigateBack`（栈空兜底 switchTab）
- `miniprogram/app.json` — tabBar.list 删 collections（剩 4 项）；pages 中保留注册
- `miniprogram/custom-tab-bar/index.js` — FULL_LIST 删 collections + 更新注释
- `miniprogram/app.wxss` — 删随之失效的 `.nav-collect` / `.nav-collect-on`
- 文档：`CLAUDE.md`（权限矩阵页签数、要点 2b、踩坑记第 4 条、页面清单与页面数）、`doc/小程序用户操作指引.md`（信息架构改四大导航、速查表、第五章入口说明、FAQ 新增「我的收藏去哪了」、截图清单）、`test/checklist.md`、`test/frontend-test-cases.md`

**变更说明**：
页签由「guest 3 / authed 4 / member 5」改为「guest 3 / authed 3 / member 4」。副作用：`minRole:'authed'` 档位不再有成员，「登录后多一个页签」的身份差异不再体现在导航上（member 仍多「我的故事」），但 RANK/minRole 机制保留（mine 用 member）。tab-bar 高亮按 pagePath 查找，删条目后其余页面 refresh 调用自动正确，无需改索引。

**验证**：
node --check 全过；app.json tabBar 剩 4 项且 pages 仍含 collections；`_tabBar` 已全清。测试：除 TYPE-A01 外全部 22 套件绿（unit 44 / 权限 17 / 分享 13 / feed 12 / 角色 16 / stats 6 等）。**TYPE-A01 失败与本次改动无关**——其断言写死种子名「巧克力工坊」，而 dev 库已由后台重命名为「手作工坊「巧克力」」，属数据漂移，未擅自改动。

### 2026-07-20 — TYPE-A01 断言改按 id 识别种子分类

**类型**：测试
**计划关联**：用户反馈（种子分类被后台改名导致断言失败，改为按 id 断言）
**修改文件**：
- `test/fn-activity-type-test.js` — TYPE-A01 由「按固定名称集 IN (...) 查 6 条」改为「按 id IN (1..6) 查 6 条」；渠道期望表改用 id 为键，报错信息带上当前名称便于定位
...
### 2026-07-23 —— 活动列表补起止时间与主理人/地点行

**类型**：前端（小程序）
**计划关联**：v2.0 真机走查反馈
**修改文件**：
- `miniprogram/pages/activities/index.js` — `_rowTime` 加结束时间；新增 `_rowSub`
- `miniprogram/pages/activities/index.wxml` / `.wxss` — 新增 `.act-rsub` 副信息行
...
```
[图标]  本与末（08-01）                    [已报名]
        08-01 周六 08:30-12:00      [醒书咖啡]
        王主理 · 线下 · 天河区体育西路 123 号
```

- **起止时间**：`_rowTime(start, end)` 同天显示 `08:30-12:00`；**跨天则结束端补日期** `08-01 周六 20:00 → 08-02 10:00`（不补日期会让人误以为当天结束）；无 `end_time` 只显开始。
- **副信息行**：主理人 · 线上/线下（线下补地点，取 `location`，无则退回 `city`）。原本挤在第二行的「线上/线下」移到这里。
- 地点可能很长，`.act-rsub` 单行截断不换行——否则卡片高度会随地址长度跳动，列表节奏乱掉。

**线上活动不取 location**：那字段存的是腾讯会议号，云函数 `list` 里已对 online 抹空，前端也只对线下取，两道都不漏。

**无需改云函数**：`organizer` / `city` / `location` / `end_time` 本就在 `LIST_SELECT` 里。
