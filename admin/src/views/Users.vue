<template>
  <div>
    ...
    <div class="filter-bar">
      ...
      <!-- 两字段语义：授权登录态与会员资格是独立维度（会员退出登录后授权态为未授权、会员资格保留） -->
      <select v-model="authState" class="select" @change="onFilter">
        <option value="">全部授权状态</option>
        <option value="guest">未授权登录</option>
        <option value="authed">已授权登录</option>
      </select>
      <select v-model="member" class="select" @change="onFilter">
    ...
    <table class="data-table">
      <thead><tr>
        <th>ID</th><th>用户</th><th>真实姓名</th><th>性别</th><th>手机号</th><th>授权登录</th><th>会员</th>
      ...
      <tbody>
        <tr v-for="u in list" :key="u.id">
          ...
          <td>{{ u.phone || '-' }}</td>
          <td><span class="badge" :class="u.authState === 'guest' ? 'badge-guest' : 'badge-authed'">{{ u.authState === 'guest' ? '未授权' : '已授权' }}</span></td>
          <td>
            <span v-if="u.isMember" class="badge badge-member">会员</span>
        ...
        <tr v-if="!list.length"><td colspan="14" class="empty">暂无用户</td></tr>
      </tbody>
    </table>
...
const list = ref([]), keyword = ref(''), authState = ref(''), member = ref('')
...
function filterParams() {
  return {
    ...
    authState: authState.value || undefined,
    member: member.value || undefined,
  }
}
async function reload() {
  const r = await getUsers({ ...filterParams(), page: page.value, pageSize: pageSize.value })
...
async function onExport() {
  ...
  const r = await getUsers({ ...filterParams(), page: 1, pageSize: 100000 })
  exportCsv(
    `醒书用户列表-${new Date().toISOString().slice(0, 10)}.csv`,
    ['用户ID', '昵称', '真实姓名', '性别', '手机号', '授权登录', '会员', '会员到期', '剩余天数', '故事数', '获赞', '收藏', '评论', '转发', '注册时间', '最后活跃'],
    r.list.map(u => [u.id, u.nickname, u.realName, genderLabel(u.gender), u.phone,
      u.authState === 'guest' ? '未授权' : '已授权', u.isMember ? '会员' : '非会员',
