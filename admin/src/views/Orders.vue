      ...
      <div class="modal">
        ...
        <div v-else-if="step===2">
          <div class="form-grid">
            ...
            <label>支付日期<input v-model="form.paymentDate" type="date" class="input-full" @change="onPaymentDate" /></label>
            <label>生效日期<input v-model="form.validFrom" type="date" class="input-full" @change="onValidFrom" /></label>
            <label>失效日期<input v-model="form.validUntil" type="date" class="input-full" @change="vuTouched = true" /></label>
          </div>
          <p class="hint-line">默认生效日为支付日期，失效日自动 +1 年，可手动修改。<span v-if="picked && picked.identity==='member'">现会员续期已默认从当前到期日（{{ picked.memberUntil || '—' }}）顺延。</span></p>
        ...
        <div v-else-if="step===3">
          <dl class="confirm-kv">
            ...
            <dt>会员有效期</dt><dd>{{ form.validFrom }} → {{ form.validUntil }}</dd>
...
// 本地日期工具（避免 toISOString 的 UTC 偏移导致日期回退一天）
...
const vfTouched = ref(false), vuTouched = ref(false)
...
function goStep2() {
  if (!vfTouched.value) {
    ...
    const renew = p && p.identity === 'member' && p.memberUntil && p.daysLeft > 0
    form.value.validFrom = renew ? p.memberUntil : form.value.paymentDate
    if (!vuTouched.value) form.value.validUntil = addYear(form.value.validFrom)
  }
  step.value = 2
}

// 支付日期变更：生效日期未被手动改过则跟随，并联动重算失效日期
function onPaymentDate() {
  if (!vfTouched.value) {
    form.value.validFrom = form.value.paymentDate
    if (!vuTouched.value) form.value.validUntil = addYear(form.value.validFrom)
  }
}
// 生效日期变更：标记已手动，失效日期未被手动改过则自动 +1 年
function onValidFrom() {
  vfTouched.value = true
  if (!vuTouched.value) form.value.validUntil = addYear(form.value.validFrom)
}
...
async function submit() {
  ...
  try {
    await createOrder({
      ...
      validFrom: form.value.validFrom, validUntil: form.value.validUntil,
    })
