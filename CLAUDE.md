...
**云函数目录**：`miniprogram/cloudfunctions/` 下共 23 个云函数——22 个小程序端（按类别分组见完整计划 `.claude/plans/fullstack-plan.md` Phase 2）+ 1 个管理端 `admin`（action 路由统一入口，供 Web 后台调用，不依赖 wx-server-sdk）。
