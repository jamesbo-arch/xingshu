...
**云函数目录**：`miniprogram/cloudfunctions/` 下共 24 个云函数——23 个小程序端（按类别分组见完整计划 `doc/fullstack-plan.md` Phase 2；含 `getStoryAudience` 作者视角人员清单）+ 1 个管理端 `admin`（action 路由统一入口，供 Web 后台调用，不依赖 wx-server-sdk）。
